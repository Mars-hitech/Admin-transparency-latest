...
@Component({
  selector: 'ngx-chat-user-show',
  templateUrl: './chat-user-show.component.html',
  styleUrls: ['./chat-user-show.component.scss']
})
export class ChatUserShowComponent implements OnInit {
  resultat:any;
  resultat_id:any;
  search:any;
  get_searc:any;
    ...
    this.route.paramMap.subscribe(async (route_params: any) => {
      if (route_params.params.id) {
        ...
        this.api.ChatUser.one(route_params.params.id)
          .get(
            {
              _includes: 'chat,user,messages',
            }
          )
          .subscribe((data) => {
            console.log(data);
           
            this.old_chat_user = data;
